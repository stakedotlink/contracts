...
/**
 * @title Espresso Fund Flow Controller
 * @notice Manages deposits and withdrawals for Espresso staking vaults
 */
contract EspressoFundFlowController is UUPSUpgradeable, OwnableUpgradeable {
    // address of staking strategy
    IEspressoStrategy public strategy;
    ...
    function shouldUnbondVaults() external view returns (bool) {
        ...
        if (
            ...
            queuedWithdrawals <= queuedDeposits
        ) return false;
    ...
    function withdrawVaults(uint256[] calldata _vaultIds) external {
        strategy.claimUnbond(_vaultIds);
    ...
    function getVaultDeposits() external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < vaults.length; ++i) {
            deposits[i] = IEspressoVault(vaults[i]).getTotalDeposits();
        }
    ...
    function getVaultRewards() external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < vaults.length; ++i) {
            rewards[i] = IEspressoVault(vaults[i]).getRewards();
        }
    ...
    function getUnbondingVaults() external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < vaults.length; ++i) {
            if (IEspressoVault(vaults[i]).isUnbonding()) {
    ...
    function getWithdrawableVaults() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < vaults.length; ++i) {
            if (IEspressoVault(vaults[i]).isWithdrawable()) {
    ...
    function getInactiveVaults() public view returns (uint256[] memory) {
        ...
        uint256 numInactiveVaults;

        for (uint256 i = 0; i < vaults.length; ++i) {
            if (!IEspressoVault(vaults[i]).isActive()) {
                inactiveVaults[i] = true;
                ++numInactiveVaults;
            }
        }

        uint256[] memory ret = new uint256[](numInactiveVaults);
        uint256 numAdded;

        for (uint256 i = 0; i < inactiveVaults.length; ++i) {
            if (inactiveVaults[i]) {
                ret[numAdded] = i;
                ++numAdded;
            }
