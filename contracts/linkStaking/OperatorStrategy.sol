...
contract OperatorStrategy is Strategy {
     ...
     */
    function depositChange() external view returns (int) {
        return _depositChange();
    }
    ...
    function updateDeposits() external onlyStakingPool returns (address[] memory receivers, uint[] memory amounts) {
        ...
        int balanceChange = _depositChange();
        if (balanceChange > 0) {
     ...
     */
    function _depositChange() internal view returns (int) {
        return
            int(
