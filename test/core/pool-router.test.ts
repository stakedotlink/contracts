...
describe('PoolRouter', () => {
  ...
  it('should return a list of all pools', async () => {
    ...
    assert.deepEqual(
      pools,
      [
        [token1.address, stakingPool1.address, 0, false],
        [token1.address, stakingPool2.address, 0, false],
        [token2.address, stakingPool3.address, 0, false],
      ],
      'pools do not match'
    )
  })
  ...
  describe('reserved allocation', async () => {
    ...
    it('maximum stake for unreserved pools should be unaffected', async () => {
      assert.equal(
        fromEther(await poolRouter.canDepositByAllowance(token1.address, 1, toEther(1))),
        10000,
        'incorrect maximum stake'
      )
      assert.equal(
        fromEther(await poolRouter.canDeposit(accounts[4], token1.address, 1)),
        10000,
        'incorrect maximum stake'
      )
    })
    ...
    it('should be able to calculate maximum stake for an arbitrary amount of allowance', async () => {
      assert.equal(
        fromEther(await poolRouter.canDepositByAllowance(token1.address, 0, toEther(600))),
        600,
        'incorrect maximum stake'
      )
      await poolRouter.setReservedModeActive(token1.address, 0, false)
      ...
      assert.equal(
        fromEther(await poolRouter.canDepositByAllowance(token1.address, 0, toEther(60))),
        10000,
        'incorrect maximum stake'
      )
    })
    ...
    it('should be able to calculate maximum stake when reserved space is 50%', async () => {
      assert.equal(
        fromEther(await poolRouter.canDeposit(accounts[3], token1.address, 0)),
        0,
        'incorrect maximum stake'
      )
      assert.equal(
      ...
      assert.equal(
        fromEther(await poolRouter.canDeposit(accounts[1], token1.address, 0)),
        300,
        'incorrect maximum stake'
      )
      assert.equal(
        fromEther(await poolRouter.canDeposit(accounts[0], token1.address, 0)),
        200,
        'incorrect maximum stake'
      )
    })
