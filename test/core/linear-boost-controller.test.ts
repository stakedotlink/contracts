...
describe('LinearBoostController', () => {
  ...
  it('boost calculation should work correctly', async () => {
    ...
    assert.equal(Number(fromEther(await boostController.getBoostAmount(toEther(5), DAY)).toFixed(5)), 0.0137)
    assert.equal(fromEther(await boostController.getBoostAmount(toEther(5), 365 * DAY)), 5)

    await boostController.setMaxBoost(6)
    assert.equal(fromEther(await boostController.getBoostAmount(toEther(5), 0)), 0)
    assert.equal(Number(fromEther(await boostController.getBoostAmount(toEther(5), DAY)).toFixed(5)), 0.02055)
    ...
    assert.equal(fromEther(await boostController.getBoostAmount(toEther(5), 0)), 0)
    assert.equal(Number(fromEther(await boostController.getBoostAmount(toEther(5), DAY)).toFixed(5)), 0.0411)
    ...
    await expect(boostController.getBoostAmount(toEther(5), 2 * 365 * DAY + 1)).to.be.revertedWith('MaxLockingDurationExceeded()')
